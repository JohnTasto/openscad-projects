...
module zigzag(bounds, holeMax=[5, 5], tiles=undef, centerLine=[true, true], wall=0.41, gap=0.03, offsets=[[],[]], alternate=false, step=0, stepStart=0) {
...
module zigzagMask(bounds, holeMax=[5, 5], tiles=undef, centerLine=[true, true], wall=0.41, gap=0.03, offsets=[[],[]], alternate=false, step=0, stepStart=0)
  if ($children > 0)
    intersection() {
      offset(delta=gap)
        zigzag(bounds, holeMax, tiles, centerLine, wall, gap, offsets, alternate, step, stepStart)
          children();
...
lineW = 0.6;
outerWall = lineW*4 + 0.02;
gridWall = lineW + 0.02;
symbolWall = lineW*2 + 0.02;

layer0 = 0.45;
layerH = 0.3;

layers = 8;
height = layer0 + layerH*(layers-1);

size = [40, 160];

gap = 0.03;

bounds = size - [1.75, 3.75];
mask = size - [outerWall*2+gap*2, outerWall*2+gap*2];
holeMax = undef;  // [1, 4];
tiles = [16, 31];
...
module backlash() {
  extrude(height)
    zigzag(bounds, holeMax, tiles, centerLine, gridWall, gap, offsets, alternate, step, stepStart)
      perimeter();
    ...
    union() {
      extrude(height)
        difference() {
          union() {
            rect(size, [0,0]);
            flipX() translate([size.x/2, 0]) {
              rotate(45) square(outerWall*1.5, center=true);
              flipY() {
                translate([0, tile.y*5 ]) rotate(45) square(outerWall, center=true);
                translate([0, tile.y*10]) rotate(45) square(outerWall, center=true);
                translate([0, tile.y*15]) rotate(45) square(outerWall, center=true);
              }
            }
          }
          perimeter();
          zigzagMask(bounds, holeMax, tiles, centerLine, gridWall, gap, offsets, alternate, step, stepStart)
            perimeter();
        }
      flipY() translate(([-size.x/2, size.y/2, 0])) rotate([90,0,0]) extrude(10)
        polygon([[gap, 0], [height-tab, 0], [-tab, height], [gap, height]]);
    }
    translate([-size.x/2-tab/2+lineW, 0, height-layerH*layers/2]) {
      translate([0, size.y/2-tab/2, 0]) extrude(height)
        rect([tab/8, tab-symbolWall*2], [0,0]);
      translate([0, tab/2-size.y/2, 0]) extrude(height) {
        rect([tab/8, tab-symbolWall*2], [0,0]);
        rect([tab-symbolWall*2, tab/8], [0,0]);
      }
    }
  }
}

// color("grey", 0.5) extrude(2) perimeter();
