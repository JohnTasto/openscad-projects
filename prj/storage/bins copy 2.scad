...
module snap(dir, d=hookD, hang=gap, trim=undef) render() translate([-dir*claspW/2, d]) {
  hookZ = is_num(trim) ? max(0, fFloor+d-trim) : undef;
  hangZ = is_num(trim) ? max(0, fFloor-hang-trim) : undef;
  rotate([180,270,0]) extrude(fWall*dir) polygon(
  [ [is_num(trim)&&trim<d ? hookZ : 0,                                       0    ]
  , [is_num(trim)&&trim<d ? hangZ : 0, is_num(trim) ? min(hang, fFloor-trim)+d : 0]
  , [is_num(trim)&&trim<d ? hangZ : 0,                                  hang+d    ]
  , [                     fGrid.z    ,                                  hang+d    ]
  , [                     fGrid.z    ,                                       0    ]
  ]);
  difference() {
    rotate([180,270,0]) extrude((fWall+hook)*dir) difference() {
      children();
      if (trim) polygon(
      [ [-fudge,          -fudge]
      , [-fudge, fWall+sPH+fudge]
      , [ hookZ, fWall+sPH+fudge]
      , [ hookZ,          -fudge]
      ]);
    }
    if (trim) translate([0, fudge, 0]) rotate([90,0,0]) extrude(d+fudge) polygon(
    [ [     (fWall-fudge)*dir,      hookZ-fudge]
    , [(hook+fWall+fudge)*dir,      hookZ-fudge]
    , [(hook+fWall+fudge)*dir, hook+hookZ+fudge]
    ]);
  }
}
...
// module tHooks()             flipX() translate([fSideIX-fSlopXY/2-claspW/2, fTopOY-fWallGrid])       hook( 1);
// module bHooks() rotate(180) flipX() translate([fSideIX-fSlopXY/2-claspW/2, fTopOY+fWallGrid-hookD]) hook(-1);
...
// module blHook(d=hookD, hang=gap) rotate(180) translate([ fSideIX-fSlopXY/2-claspW/2, fTopOY+fWallGrid-hookD]) hook(-1, d, hang);
// module brHook(d=hookD, hang=gap) rotate(180) translate([-fSideIX+fSlopXY/2+claspW/2, fTopOY+fWallGrid-hookD]) hook( 1, d, hang);

module tSnaps()             flipX() translate([fSideIX-fSlopXY/2-claspW/2, fTopOY-fWallGrid])       snap( 1) dent();
module bSnaps() rotate(180) flipX() translate([fSideIX-fSlopXY/2-claspW/2, fTopOY+fWallGrid-hookD]) snap(-1, trim=claspD-fWall+fSlopXY/2) bump();
module lSnaps() rotate(90)  flipX() translate([fTopOY-claspW/2, fSideOX]) snap( 1) dent();
module rSnaps() rotate(270) flipX() translate([fTopOY-claspW/2, fSideOX]) snap(-1, trim=claspD+fSlopXY) bump();

module blSnap() rotate(180) translate([ fSideIX-fSlopXY/2-claspW/2, fTopOY+fWallGrid-hookD]) snap(-1, fWall2, claspD-fWall2, trim=fWallGrid) bump();
module brSnap() rotate(180) translate([-fSideIX+fSlopXY/2+claspW/2, fTopOY+fWallGrid-hookD]) snap( 1, fWall2, claspD-fWall2, trim=fWallGrid) bump();

// snap(-1) bump();
// snap(1, fWall2, claspD-fWall2, trim=3) bump();
// bSnaps();



module tlSeamFill(l) translate([fGrid.x*l-fBulgeOX, claspD-fTopOY]) {
  rB = [fBulgeWall+fWallGrid, -claspD];
  rL = rB - [fWall, -fWall];
...
module bBase(l=0, r=0) if (l<=r) for (i=[l:r]) translate([fGrid.x*i, 0]) {
  extrude(fFloor) translate([0, fTopOY-fWall]) rect([fSideOX*2-fWallGrid*4, claspD], [0,1]);
...
module bSide(l=0, r=0) {
  if (l<=r) {
    extrude(fFloor) translate([fGrid.x*l-fBulgeOX, fTopOY]) rect([fGrid.x*(r-l)+fBulgeOX*2, -fWall2]);
...
module lSide(b=0, t=0) if (b<=t) {
  ...
  extrude(fFloor) for (i=[b:t]) if (i!=b) translate([fSideIX, fGrid.y*(i-0.5)]) rect([claspD+fWallGrid, fWall2], [1,0]);
  for (i=[b:t]) translate([fSideIX, fGrid.y*i]) {
    rB = [fBulgeWall, fTopOY*2-claspW*2-fSlopXY];
    rL = rB - [fWall, 0];
...
module rSide(b=0, t=0) if (b<=t) {
  extrude(fFloor) for (i=[b:t]) translate([fGrid.x*0-fSideIX, fGrid.y*i]) flipY() translate([0, fTopOY-fWallGrid]) rect([-claspD-fWallGrid, -claspW+fWallGrid]);
  extrude(fGrid.z) translate([-fSideOX, fGrid.y*b-fTopOY]) rect([fWall, fGrid.y*(t-b)+fTopOY*2]);
  for (i=[b:t]) translate([0, fGrid.y*i]) lSnaps();
  for (i=[b:t]) translate([-fSideIX, fGrid.y*i]) {
    rB = [-fBulgeWall, fTopOY*2-claspW*2+fWall*2];
    rL = rB - [-fWall, fWall*2];
    extrude(fFloor) rect(rB, [1,0]);
    translate([0, 0, fGrid.z-top]) difference() {
      hull() {
...
module blCorner(t=1, r=1) {
  ...
  fillet = fWallGrid*2;
  if (1 <= r) {
    extrude(fFloor) translate([fGrid.x-edge, fTopOY]) rect([fGrid.x*(r-1)+fBulgeOX+edge, -fWall2]);
    extrude(fGrid.z) {
...
module brCorner(t=1, l=-1) {
  ...
  fillet = fWallGrid*2;
  if (l <= -1) {
    extrude(fFloor) translate([edge-fGrid.x, fTopOY]) rect([fGrid.x*(l+1)-fBulgeOX-edge, -fWall2]);
    extrude(fGrid.z) {
...
module frame(t=0, b=0, l=0, r=0) {
  ...
  module bulge() extrude(fGrid.z) translate([0, fBulgeOX]) {
    flipX() translate([fBulgeOY, 0]) rect([-fWall, -fBulgeWall]);
  ...
  module bFrame(l=0, r=0) {
    translate([fGrid.x*l, fWall+fWallGrid-hookD]) blSnap();
    translate([fGrid.x*r, fWall+fWallGrid-hookD]) brSnap();
    extrude(fGrid.z) {
  ...
  extrude(fFloor) {
    translate([fGrid.x*r+fSideOX, fGrid.y*t+fTopOY]) rect([fGrid.x*(l-r)-fSideOX*2, fGrid.y*(b-t)-fTopOY*2+claspD-fWall]);
    ...
    for (i=[b:t]) translate([fGrid.x*l-fSideIX, fGrid.y*i]) flipY() translate([0, fTopOY-fWallGrid]) rect([-claspD-fWallGrid, -claspW+fWallGrid]);

    for (i=[b:t]) translate([fGrid.x*l-fBulgeOX, fGrid.y*i]) rect([fBulgeWall, fTopOY*2-claspW*2+fWall*2], [1,0]);
    for (i=[b:t]) translate([fGrid.x*r+fBulgeOX, fGrid.y*i]) rect([-fBulgeWall, fBulgeOY*2], [1,0]);

    for (i=[l:r]) translate([fGrid.x*i, fGrid.y*t+fTopOY-fWall]) rect([fSideOX*2-fWallGrid*4, claspD], [0,1]);
...
module demoFrameSmall() {
  color([0.6,0.6,0.6,1])
    rotate([90]) {
      translate([0,0,300]) frame();
      translate([-fGrid.x/2, -fGrid.y/2, 150]) frame(t=1, b=0, l=0, r=1);

      frame(t= 0, b=-1, l=-1, r= 0);
      frame(t= 1, b= 1, l=-1, r= 0);
      frame(t= 0, b=-1, l= 1, r= 1);
      frame(t= 1, b= 1, l= 1, r= 1);
    }
  color([0.85,0.6,0.0,1])
    rotate([90]) {
      perimeter(t=1, b=-1, l=-1, r=1);
  }
}
