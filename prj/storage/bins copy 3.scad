...
//     - widen the hook part?
//   [ ] instead of t/b/f/b/l/r, make everything instead take w, d, & h
...
dFaceD = dWall2 + gap + dFloat;  // how far the sides should overlap the drawer faces
...
function fillAdjW    (w, flushSides, wall2) = abs(w) - (1-flushSides)*gap;
function fillWalls   (w, flushSides, wall2) = div(fillAdjW(w, flushSides, wall2) + fudge, wall2+gap);  // `fudge` compensates for some FP precision errors
function fillExtraW  (w, flushSides, wall2) = max(0, fillAdjW(w, flushSides, wall2) - fillWalls(w, flushSides, wall2)*(wall2+gap));  // ditto for `max(0, ...)`
function fillExtraGap(w, flushSides, wall2) = fillExtraW(w, flushSides, wall2) / (fillWalls(w, flushSides, wall2)*2 + 1-flushSides);
...
module tBase(l=0, r=0) if (l<=r) for (i=[l:r]) translate([fGridX*i, 0]) {
  bHooks();
...
module bBase(l=0, r=0) if (l<=r) for (i=[l:r]) translate([fGridX*i, 0]) {
...
module tlCorner(b=-1, r=1) {
  ...
  if (1 <= r) {
    extrude(fGridZ+dFaceD) {
      translate([fGridX-edge, fBHookY+bPH]) rect([fGridX*(r-1)+fBulgeOX+edge, fWall2]);
...
module trCorner(b=-1, l=-1) {
  ...
  if (l <= -1) {
    extrude(fGridZ+dFaceD) {
      translate([edge-fGridX, fBHookY+bPH]) rect([fGridX*(l+1)-fBulgeOX-edge, fWall2]);
      corner(edge, fillet, [-1,-1]);
    }
    tlSeamFill(l);
...
module blCorner(t=1, r=1) {
  ...
  if (1 <= r) {
    ...
    extrude(fGridZ+dFaceD) {
      translate([fGridX-edge, fTHookY]) rect([fGridX*(r-1)+fBulgeOX+edge, -fWall2]);
      corner(edge, fillet, [1,1]);
    }
    brSeamFill(r);
...
module brCorner(t=1, l=-1) {
  ...
  if (l <= -1) {
    ...
    extrude(fGridZ+dFaceD) {
      translate([edge-fGridX, fTHookY]) rect([fGridX*(l+1)-fBulgeOX-edge, -fWall2]);
      corner(edge, fillet, [-1,1]);
    }
    blSeamFill(l);
...
module frame(t=0, b=0, l=0, r=0, drawer=false, drawFace=true, drawTop=true, drawFloor=true, drawSides=true) {
  ...
  if (drawer || is_num(drawer))
    translate([0, fGridY*b+fBotIY+dSlopZ/2, drawerY/2+dFloat+fFloor+fGridZExtra+fBulgeWall+(is_num(drawer)?drawer:0)])
      rotate([-90,0,0]) drawer(h=t-b+1, l=l, r=r, drawFace=drawFace);
}
...
module drawer(h=1, l=0, r=0, drawFace=true) {
...
module bin(h=1, f=0, b=0, l=0, r=0) {
  binR = binXY;
  w = bGridXY*(r-l) + binXY;
  d = bGridXY*(b-f) + binXY;
  if (h>=1) translate([bGridXY*(r+l)/2, bGridXY*(b+f)/2, bFloor+binR*sqrt(2)/2]) {
...
module demoPerimeter(t=0, b=0, l=0, r=0, cornerWalls=1) {
  translate([0, fGridY*(t+1)]) tSide(l+cornerWalls, r-cornerWalls);
  translate([0, fGridY*(b-1)]) bSide(l+cornerWalls, r-cornerWalls);
  translate([fGridX*(l-1), 0]) lSide(b+cornerWalls, t-cornerWalls);
  translate([fGridX*(r+1), 0]) rSide(b+cornerWalls, t-cornerWalls);
  translate([fGridX*(l-1), fGridY*(t+1)]) tlCorner(b=-cornerWalls, r= cornerWalls);
  translate([fGridX*(r+1), fGridY*(t+1)]) trCorner(b=-cornerWalls, l=-cornerWalls);
  translate([fGridX*(l-1), fGridY*(b-1)]) blCorner(t= cornerWalls, r= cornerWalls);
  translate([fGridX*(r+1), fGridY*(b-1)]) brCorner(t= cornerWalls, l=-cornerWalls);
}
...
module demoFrameSmall(drawers=true) {
  ...
  color([0.6,0.6,0.6,1])
    rotate([90]) {
      ...
      translate([-fGridX/2-100, -fGridY/2, 150]) frame(t=1, b=0, l=0, r=1);
      translate([-fGridX*3/2, -fGridY*3/2, 0]) {
        frame(t=3, b=3, l=0, r=0, drawer=drawers);
        frame(t=2, b=1, l=0, r=0, drawer=drawers);
        frame(t=0, b=0, l=0, r=0, drawer=drawers);

        frame(t=3, b=3, l=1, r=2, drawer=drawers);
        frame(t=2, b=1, l=1, r=2, drawer=drawers);
        frame(t=0, b=0, l=1, r=2, drawer=drawers);

        frame(t=3, b=3, l=3, r=3, drawer=drawers);
        frame(t=2, b=1, l=3, r=3, drawer=drawers);
        frame(t=0, b=0, l=3, r=3, drawer=drawers);
      }
    }
  color([0.85,0.6,0.0,1])
    rotate([90]) {
      translate([-fGridX*3/2, -fGridY*3/2, 0])
        demoPerimeter(t=3, b=0, l=0, r=3);
  }
}
...
module demoFrameLarge(drawers=true) {
  ...
  color([0.6,0.6,0.6,1])
    rotate([90]) {
      ...
      frame(t= 2, b= 2, l=-4, r=-3, drawer=drawers? 0  :false);
      frame(t= 1, b= 1, l=-4, r=-3, drawer=drawers? 5  :false);
      frame(t= 0, b= 0, l=-4, r=-3, drawer=drawers?10  :false);
      frame(t=-1, b=-2, l=-4, r=-3, drawer=drawers? 2.5:false);

      frame(t= 2, b= 2, l=-2, r=-2, drawer=drawers? 5  :false);
      frame(t= 1, b= 1, l=-2, r=-2, drawer=drawers?10  :false);
      frame(t= 0, b= 0, l=-2, r=-2, drawer=drawers?20  :false);
      frame(t=-1, b=-2, l=-2, r=-1, drawer=drawers? 7.5:false);

      frame(t= 2, b= 2, l=-1, r= 1, drawer=drawers?10:false);
      frame(t= 1, b= 1, l=-1, r= 1, drawer=drawers?20:false);
      frame(t= 0, b= 0, l=-1, r= 1, drawer=drawers?40:false);

      frame(t=-1, b=-1, l= 0, r= 0, drawer=drawers?20:false);
      frame(t=-2, b=-2, l= 0, r= 0, drawer=drawers?10:false);

      frame(t= 2, b= 2, l= 2, r= 2, drawer=drawers? 5  :false);
      frame(t= 1, b= 1, l= 2, r= 2, drawer=drawers?10  :false);
      frame(t= 0, b= 0, l= 2, r= 2, drawer=drawers?20  :false);
      frame(t=-1, b=-2, l= 1, r= 2, drawer=drawers? 7.5:false);

      frame(t= 2, b= 2, l= 3, r= 4, drawer=drawers? 0  :false);
      frame(t= 1, b= 1, l= 3, r= 4, drawer=drawers? 5  :false);
      frame(t= 0, b= 0, l= 3, r= 4, drawer=drawers?10  :false);
      frame(t=-1, b=-2, l= 3, r= 4, drawer=drawers? 2.5:false);
    }
  color([0.85,0.6,0.0,1])
    rotate([90]) {
      demoPerimeter(t=2, b=-2, l=-4, r=4, cornerWalls=2);
  }
}

module demoDrawerBumpAlignment(t=0, b=0, l=0, r=0, drawer=dTravel)
  rotate([90,0,0]) translate([0, -fBotIY-dSlopZ/2, -drawerY/2-dFloat-fFloor-fGridZExtra-fBulgeWall]) {
    frame(t=t, b=b, l=l, r=r, drawer=drawer, drawFace=false, drawFloor=false, drawSides=false);
    translate([0, fGridY*(b-t-1), 0]) frame(t=t, b=b, l=l, r=r, drawTop=false, drawFloor=false, drawSides=false);
  }

module demoDrawerZAlignment(t=0, b=0, l=0, r=0)
  rotate([90,0,0]) translate([0, -fBotIY-dSlopZ/2, -drawerY/2-dFloat-fFloor-fGridZExtra-fBulgeWall])
    frame(t=t, b=b, l=l, r=r, drawer=true, drawFace=false, drawFloor=false);
...
// color([0.5,0.5,0.5,1]) extrude(fGridZ) demoPerimeter();
// color([0.5,0.5,0.5,1]) extrude(fGridZ) demoPerimeter(t=1, b=-2, l=-1, r=2);
...
// frame();
// frame(drawer=true);
// frame(t=1, b=-1, l=-1, r=1, drawer=true);
// frame(t=1, r=0);
// frame(t=1, drawer=true);

// demoDrawerBumpAlignment(t=1);
// demoDrawerZAlignment(t=1);
