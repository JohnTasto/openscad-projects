...
General
  ...
  [ ] consider requiring h > 1
    - can h = 2 be made to stay under 33 tall for ditto printing?
Bumps
  [ ] parameterize bottom bump width
Frame
  [x] hook snaps
  [x] drawer snaps
  [x] drawer stops
  ...
  [ ] mounting holes
  [ ] move Y hooks down so horn is taller instead of extra hang on bottom hooks
  [ ] make `hook` adjustable per axis
Drawers
  [x] drawers
  [x] wings
  [x] ruffles
  ...
  [x] snaps
Handles
  [ ] make "upright" handle work for hL<hH/2
  [x] draw "loop" and "tray" handles
  [ ] make "loop" and "tray" handles double walled
Bins
  [x] bins
Sides
...
handleType = "loop";  // "upright", "tray", "loop", or "nose"
handleReach = 15;
handleLip = 5;
handleHeight = dLayerAbsFloor(fGridY - dSlopZ);  // ignored by "tray" handle type
...
module tHooks(drawHooks=true) flipX() translate([fSideIX-fSlopXY-claspW/2, fTHookY, 0]) {
  translate([-claspW/2, hookD, fGridZ-bIL-bFL]) hull() {
    translate([0, 0, bFL]) box([fWall2, -fudge, -bFL-bPL-bBL]);
    box([fWall2, bPH, -bPL]);
  };
...
module tlSide(x=1, z=[0], trim=false) {
  ...
  y = z[0];
  if (l<=r) {
    fillet = (tPH > 0 ? fWallGrid : claspD) + tClearance;
    rise = tPH > 0 ? claspD - fWallGrid + fSlopXY : fSlopXY;
    edge = (tPH>0?claspD+fSlopXY:fWall2+fSlopXY*2)+fSideOX+stretchX-tClearance;
    align = [1, -1];
    translate([fGridX*(l-1), fGridY*(y+1), 0]) {
      ...
      extrude(fGridZ+dFaceD) {
        ...
        cornerWall(fillet, [edge, fHornY+rise], align, trim=[undef, rise-fSlopXY]);
        if (tPH>0) translate([fGridX-edge-fillet, fBHookY-fSlopXY]) rect([fWall2, -rise+fSlopXY]);
      }
      trSeamFill(r-l+1);
  ...
  if (l<=r) {
    ...
    translate([fGridX*(r+1), fGridY*(y+1), 0]) {
      ...
      extrude(fGridZ+dFaceD) {
        ...
        cornerWall(fillet, [edge, fHornY+rise], align, trim=[undef, rise-fSlopXY]);
        if (tPH>0) translate([edge-fGridX+fillet, fBHookY-fSlopXY]) rect([-fWall2, -rise+fSlopXY]);
      }
      tlSeamFill(l-r-1);
      rB = [-claspD-stretchX-fWallGrid*3-lPC-lWS, -rise-fillet+fSlopXY];
      rL = rB - [-fWall2*2-lWS, -fWall2];
...
module blSide(x=1, z=[0], trim=false) {
  ...
  if (l<=r) {
    fillet = fWall4;
    edge = fGridX/2+claspD/2+stretchX/2-fWall2;
    align = [1, 1];
    translate([fGridX*(l-1), fGridY*(y-1), 0]) {
      bSideBase(1, r-l+1);
      extrude(fGridZ+dFaceD) {
      ...
      brSeamFill(r-l+1);
      rB = [claspD+stretchX+fWallGrid*2, fillet];
      rL = rB - [fWall2*2, +fWall2];
      tx = fGridX-edge-fillet;
  ...
  if (l<=r && h>=1) translate([fGridX*(r+l)/2, 0, 0]) {
    difference() {
      ...
      if (r>=l) for (i=[l:r]) translate([fGridX*i-fGridX*(r+l)/2, dFloat, 0]) flipX() translate([fSideIX-fSlopXY-claspW-dSlopXY, 0, 0]) hull() {
        translate([0, -drawerY/2+dTravel+bIL+bFL+bPL+bBL*(bSH/bPH), 0]) {
    ...
    if (handleReach>0) {
      ...
      if (handleType=="tray" || handleType=="loop") {
        // fullHeight = dLayerAbsFloor(handleLip/2 + handleLip*sqrt(2)/2 - dWall2/2 + dLayerHN/2);
        fullHeight = dLayerAbsFloor(handleLip/2 + handleLip*sqrt(2)/2 - dWall2*3/8 + dLayerHN/2);
        // hH = (fullHeight*2 + dWall2 - dLayerHN)/(1 + sqrt(2));  // derived by removing `dLayerAbsFloor` from above and solving for `handleLip`
        hH = (fullHeight*2 + dWall2*3/4 - dLayerHN)/(1 + sqrt(2));  // derived by removing `dLayerAbsFloor` from above and solving for `handleLip`
        // trunc = hH*sqrt(2)/2 - dWall2/2 + dLayerHN/2;
        trunc = hH*sqrt(2)/2 - dWall2*3/8 + dLayerHN/2;
        a = (w-drawerX+fBulgeOX*2)/2 - hH/2;
        b = handleReach - hH/2;
        step = 360/$fn;
        translate([0, -drawerY/2-gap, 0]) {
          difference() {
            translate([0, -dWall2, 0]) {
              translate([0, 0, hH/2]) rotate(180) for (i=[0:step:180-step]) hull() {
                translate([a*cos(i), b*sin(i), 0]) rotate(atan2(b*cos(i), -a*sin(i))) horizontalHandleSegment(hH, trunc, handleType=="tray");
                translate([a*cos(i+step), b*sin(i+step), 0]) rotate(atan2(b*cos(i+step), -a*sin(i+step))) horizontalHandleSegment(hH, trunc, handleType=="tray");
              }
              if (handleType=="tray") extrude(dFloor) scale([a, b]) circle();
            }
            translate([0, 0, -fudge]) box([w-drawerX+fBulgeOX*2, handleReach, hH/2+trunc+fudge2], [0,1,1]);
            translate([0, -dWall2, dFloor]) for (i=[0:dLayerRelFloor(hH/2+trunc-dFloor)/dLayerHN]) translate([0, 0, i*dLayerHN]) {
              dir = mod(i, 2) == 0 ? 1 : -1;
              box([(a+hH/2+fudge)*dir*-1, -gap, dLayerHN+fudge]);
              box([(a-dWall2/2)*dir, -gap, dLayerHN+fudge]);
              translate([(a+dWall2/2)*dir, 0, 0]) box([(hH/2-dWall2/2+fudge)*dir, -gap, dLayerHN+fudge]);
            }
          }
        }
      }
